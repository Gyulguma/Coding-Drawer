...
String에서 substring()을 사용하면서 'java.lang.StringIndexOutOfBoundsException' 런타임 에러가 발생했다. 이는 자르려는 문자열의 길이가 원본 문자열의 길이보다 길거나 맞지 않을 때 발생하는 에러다. 이를 예방하기 위해서는 if문으로 원본 문자열의 길이를 판단해 substring을 사용하는 방법이 있다. 이번 문제에서는 substring()을 사용하지 않고 replace()를 대신 사용하여 문제를 해결하였다. 코드를 작성할 때 예외상황을 항상 생각하며 작성하는 것이 중요하다.
